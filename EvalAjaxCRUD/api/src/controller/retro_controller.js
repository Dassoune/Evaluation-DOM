...
exports.getDataForretro = (request, response) => {
  ...
  fs.readFile("./src/model/data.json/", (err, data) => {
    ...
    if (err) {
      ...
      // Sinon on renvoie le contenu du tableau retro
      response.status(200).json(JSON.parse(data).retro);
...
exports.getDataByIdForretro = (request, response) => {
  // Lecture du contenu du fichier
  fs.readFile("./src/model/data.json/", (err, data) => {
    // Si il y a une erreur dans la lecture on renvoie une erreur avec un message
    if (err) {
      ...
      // On cherche dans le tableau retro si l'id demandé existe
      const dataById = jsonData.retro.find(
...
exports.getDataByNameForretro = (request, response) => {
  // Lecture du contenu du fichier
  fs.readFile("./src/model/data.json/", (err, data) => {
    // Si il y a une erreur dans la lecture on renvoie une erreur avec un message
    if (err) {
      ...
      // Sinon on stock les données existante dans une constante
      const jsonData = JSON.parse(data);
      // On cherche dans le tableau retro si le nom demandé existe
      const dataByName = jsonData.retro.find(
...
exports.createDataForretro = (request, response) => {
  // Lecture du contenu du fichier
  fs.readFile("./src/model/data.json/", (err, data) => {
    // Si il y a une erreur dans la lecture on renvoie une erreur avec un message
    if (err) {
      ...
      // Sinon on stock la longueur+1 du tableau retro dans une constante
      const newID =
        existingData.retro[existingData.retro.length - 1].id + 1;
      ...
      // On push le nouvel objet dans le tableau retro
      existingData.retro.push(newGame);
      fs.writeFile(
        "./src/model/data.json/",
...
exports.updateDataForretro = (request, response) => {
  // Lecture du contenu du fichier
  fs.readFile("./src/model/data.json", (err, data) => {
    // Si il y a une erreur dans la lecture on renvoie une erreur avec un message
    if (err) {
      ...
      const existingData = JSON.parse(data);
      // On cherche dans le tableau retro si l'id demandé existe
      const dataById = existingData.retro.find(
        (obj) => obj.id === parseInt(request.params.id)
      );
      if (!dataById) {
        ...
        // On écrit les modifications dans le tableau retro
        fs.writeFile(
          "./src/model/data.json",
          JSON.stringify(existingData),
          (writeErr) => {
...
exports.deleteDataForretro = (request, response) => {
  // Lecture du contenu du fichier
  fs.readFile("./src/model/data.json", (err, data) => {
    // Si il y a une erreur dans la lecture on renvoie une erreur avec un message
    if (err) {
      ...
      if (!dataById) {
        ...
        existingData.retro = existingData.retro.filter(
          (obj) => obj.id != parseInt(request.params.id)
        );
        // On écrit les modifications dans le tableau retro
        fs.writeFile(
